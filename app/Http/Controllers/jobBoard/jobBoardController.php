...
class jobBoardController extends Controller
{
    ...
    {
        try {
            $get = jobBoardOrders::with(['user', 'services.maintenance.skill', 'services.housekeeping.skill', 'services.rentafriend.skill'])->whereNull('status')->paginate(10);
            return response()->json([
                'data' => $get,
            ], 200);
        } catch (\Exception $e) {
    ...
    public function searchJobBoard(Request $request)
    {
        try {
            ...
            $search = jobBoardOrders::where([["service_name", "LIKE", "%" . ($validate_query["service_name"] ?? '') . "%"]])
                ->with(['user', 'services.maintenance.skill', 'services.housekeeping.skill', 'services.rentafriend.skill'])
